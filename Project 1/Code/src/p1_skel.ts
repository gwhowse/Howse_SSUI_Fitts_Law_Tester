...
class Target extends ScreenObject{
    ...
    newGeom(newCentX : number, newCentY : number, newDiam? : number) {
        ...
        // === YOUR CODE HERE ===
        //update diam if we need to
        if (!(typeof(newDiam) === "undefined")) {
            this.diam = newDiam;
        }

        //update the coordinates of our bounding box
        this.x = newCentX - (this.diam / 2);
        this.y = newCentY - (this.diam / 2);
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        // check that r >= sqrt((x2-x1)^2 + (y2-y1)^2)
        const xDiffSquared = (ptX - this.centerX) ** 2;
        const yDiffSquared = (ptY - this.centerY) ** 2;
        return (xDiffSquared + yDiffSquared) <= (this.radius ** 2);
    }
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        //ignore if not visible or click is out of bounds
        if (!this.visible || !this.pickedBy(ptX, ptY)) return false;

        this.parentUI.recordTrialEnd(this.centerX, this.centerY, this.radius);
...
class Reticle extends Target {
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        //inner circle
        ctx.beginPath();
        ctx.arc(this.centerX, this.centerY, Reticle.RETICLE_INNER_DIAM, 0, 2 * Math.PI);
        ctx.stroke();
    }
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        const xDiffSquared = (ptX - this.centerX) ** 2;
        const yDiffSquared = (ptY - this.centerY) ** 2;
        return (xDiffSquared + yDiffSquared) <= (Reticle.RETICLE_INNER_DIAM ** 2);
    }
...
class BackgroundDisplay extends ScreenObject{
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        // === YOUR CODE HERE ===
        //loop through all three messages, drawing each if they aren't empty
        let msgs = [this.msg1, this.msg2, this.msg3];
        for (let msgIndex = 0; msgIndex < 3; msgIndex++) {
            if (!(msgs[msgIndex] === "")) {
                ctx.fillText(msgs[msgIndex], xpos, ypos);
                ypos += fontHeight;
            }
        }
    }
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        // ignore if not visible or if we aren't in 'start' state
        if (!this.visible || !(this.parentUI.currentState === "start")) return false;
